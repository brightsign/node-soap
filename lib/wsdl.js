 ...
 * MIT Licensed
 *
 * sinai: copied from soap module so I can check it in
 */
/*jshint proto:true*/
...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  if (root.Envelope) {
    ...
    if (body.Fault) {
      // jsugg - mod to handle faultstring element with attributes, which returns an object, not a string
      var faultString = body.Fault.faultstring;
      if (faultString != null && typeof(faultString) == "object") {
        faultString = faultString[self.options.valueKey];
      }
      // jsugg - return faultSring only
      // TODO - make this an option
      var error = new Error(faultString);
      //var error = new Error(body.Fault.faultcode + ': ' + faultString + (body.Fault.detail ? ': ' + body.Fault.detail : ''));
      error.root = root;
      throw error;
    }
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  if (Array.isArray(obj)) {
    if (obj.length == 0) {
      // sinai: added this if
      var correctNamespace = parentNamespace || ns; //using the parent namespace if given
      parts.push(['<', correctNamespace, name, ' xsi:nil="true"', xmlnsAttrib, '>'].join(''));
      parts.push(['</', correctNamespace, name, '>'].join(''));
    } else {
      ...
      //}
      // sinai: move the array wrapper so it only appears once.
      parts.push(['<', "", name, "", xmlnsAttrib, '>'].join(''));
      for (var i = 0, item; item = obj[i]; i++) {
